...
def parse_scene_heuristically(scene: Scene) -> ScreenplayResponse:
    ...
    precyzyjnie rozdzielający kwestie mówione od wtrąceń narracyjnych
    i przypisujący odpowiednie role w obsadzie.
    """
    ...
    for p in paragraphs:
        ...
        if p.startswith(("—", "–", "-")):
            ...
            # Kwestia wypowiadana przez postać
            cues.append(
                AudioCue(
            ...
            # Jeśli w dialogu było wtrącenie narracyjne (np. "Ukłoniłam się lekko..."), dodaj je jako kwestię narratora
            if narrative_tag:
                cues.append(
                    AudioCue(
                        cue_id=f"{scene.scene_id}_{order:03d}",
